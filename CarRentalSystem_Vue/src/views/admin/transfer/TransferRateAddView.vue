...
const submitForm = async () => {
  try {
    await request.post('/transferRate', rate.value)
    alert('新增成功')
...
const submitForm = async () => {
  try {
    await axios.post('http://localhost:8081/api/transferRate', rate.value)
    alert('新增成功')
    router.push('/transferRate')
  } catch (error) {
    console.error(error)
  ...
  <div class="container-fluid py-4">
    <div class="card border-0 shadow-sm mb-4 bg-primary bg-gradient text-white rounded-4">
      <div class="card-body p-4">
        <h2 class="fw-bold mb-1"><i class="fa-solid fa-money-check-dollar me-2"></i>新增費率</h2>
        <p class="mb-0 text-white-50">Create New Transfer Rate</p>
      </div>
    </div>
