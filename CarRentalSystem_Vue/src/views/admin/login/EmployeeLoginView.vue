...
const handleLogin = async () => {
  ...
  try {
    ...
    const res = await authAPI.employeeLogin(account.value, password.value)

    if (res.data.success) {
     //  重點修改區塊開始：儲存 Token 
      // 請依據後端的實際 JSON 欄位名稱調整，後端把 token 放在 res.data.token
      const token = res.data.token
      if (token) {
        localStorage.setItem('employee_token', token)
        localStorage.setItem('role', res.data.role) // 可選：儲存使用者角色，方便前端判斷
        localStorage.setItem('employee', JSON.stringify(res.data.employee)) // 儲存員工資料（含 empId）
      ...
      // 登入成功：導向後台首頁（取代原本的 response.sendRedirect）
      ...
      <form @submit.prevent="handleLogin">
        ...
        <div class="d-flex gap-2 mb-4">
          <button 
            ...
            @click="account='admin001'; password='hashed_pw_001'"
          >
            <i class="fa-solid fa-user-shield me-1"></i>Demo Admin
          </button>
          <button 
            type="button" 
            class="btn btn-outline-success flex-fill" 
            style="font-size: 0.85rem; padding: 0.4rem;" 
            @click="account='staff001'; password='hashed_pw_002'"
          >
            <i class="fa-solid fa-user-tie me-1"></i>Demo Employee
          </button>
        </div>
