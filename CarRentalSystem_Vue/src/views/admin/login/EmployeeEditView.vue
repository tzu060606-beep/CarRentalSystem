...
// 取得目前登入的員工 ID，用了防止自己修改自己的資料
const currentEmpId = (() => {
  const stored = localStorage.getItem('employee')
  return stored ? JSON.parse(stored)?.empId : null
})()
...
const defaultForm = () => ({
  ...
  empEmail:   '',
  empStatus:  '在職',    // 預設為在職
  resignDate: null       // 離職日期，在職時為 null
})
...
const handleDelete = async (empId, empName) => {
  if (!confirm(`您確定要將「${empName}」標記為離職嗎？此動作會記錄離職日期。`)) return
  try {
    await employeeAPI.delete(empId)
    showSuccess(`員工「${empName}」已標記為離職`)
    fetchEmployees()
  } catch (err) {
...
const statusLabel = (s) => s === '在職' ? '在職' : '離職'
const statusBadge = (s) => s === '在職' ? 'bg-success' : 'bg-secondary'
</script>
          ...
          <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
              <tr v-for="emp in employees" :key="emp.empId"
                :class="emp.empId === currentEmpId ? 'table-secondary' : ''"
              >
                <td class="text-secondary">{{ emp.empId }}</td>
                <td class="fw-medium">
                  {{ emp.empName }}
                  <span v-if="emp.empId === currentEmpId" class="badge bg-secondary ms-1" style="font-size: 0.7rem;">本人</span>
                </td>
                ...
                <td class="text-center">
                  <button
                    ...
                    :disabled="emp.empId === currentEmpId"
                    :title="emp.empId === currentEmpId ? '無法修改自己的資料' : ''"
                  >
                  ...
                  <button
                    ...
                    :id="`btn-resign-${emp.empId}`"
                    :disabled="emp.empStatus === '離職' || emp.empId === currentEmpId"
                    :title="emp.empId === currentEmpId ? '無法將自己標記為離職' : ''"
                  >
                    <font-awesome-icon :icon="['fas', 'right-from-bracket']" class="me-1" />
                    離職
                  </button>
                </td>
              </tr>
          ...
          <div class="modal-body">
            ...
            <form @submit.prevent="saveEmployee" id="form-employee">
              <div class="row g-3">
                ...
                <!-- 離職日期（修改模式且狀態為「離職」時才顯示） -->
                <div v-if="isEditing && formData.empStatus === '離職'" class="col-md-6">
                  ...
                  <input
                    ...
                    id="input-resignDate"
                  />
                  <div class="form-text">留空則自動記錄今日日期</div>
                </div>
